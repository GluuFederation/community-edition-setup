idp.attribute.resolver.datasource.driverClass=%(rdbm_driver_class)s
idp.attribute.resolver.datasource.jdbcUrl=jdbc:%(rdbm_name)s://%(rdbm_host)s:%(rdbm_port)s/%(rdbm_db)s
